...
import numpy as np
from mcpi import block, event, minecraft
...
class MinecraftPiDoom:
    ...
    def __init__(self) -> None:
        ...
        self._ctrls_pos = Vec3(160 // self._scale, 100 // self._scale - 2,
                               68 - 6 * self._scale)
        self._mc.setBlocks(0, 0, 0, 256, 128, 128, block.AIR)
        self._mc.setBlocks(self._ctrls_pos.x - 2, self._ctrls_pos.y + 1,
                           self._ctrls_pos.z + 2, self._ctrls_pos.x + 2,
                           self._ctrls_pos.y + 2, self._ctrls_pos.z - 3,
                           block.BEDROCK_INVISIBLE)
        self._mc.setBlocks(self._ctrls_pos.x - 1, self._ctrls_pos.y + 1,
                           self._ctrls_pos.z + 1, self._ctrls_pos.x + 1,
                           self._ctrls_pos.y + 2, self._ctrls_pos.z - 2,
                           block.AIR)
        for blk in PLATFORM:
            self._mc.setBlock(self._ctrls_pos.x + blk[0],
                              self._ctrls_pos.y + blk[1],
                              self._ctrls_pos.z + blk[2], blk[3])
        self._mc.player.setTilePos(self._ctrls_pos + Vec3(0, 1, 0))

    def draw_frame(self, pix: np.ndarray) -> None:
        for y, x in itertools.product(range(0, 200, self._scale),
                                      range(0, 320, self._scale)):
            idx = np.argmin(
                color.deltaE_cie76(
                    PALETTE_LAB,
                    color.rgb2lab(pix[199 - y, x, [2, 1, 0]] / 255)))
            self._mc.setBlock(x // self._scale, y // self._scale, 0,
                              PALETTE_BLOCKS[idx])
    ...
    def get_key(self) -> Optional[tuple[int, int]]:
        if not self._read_frame_input:
            ...
            self._inputs = ([(0, key)
                             for key in iter(self._pressed - cur_pressed)] +
                            [(1, key)
                             for key in iter(cur_pressed - self._pressed)])
